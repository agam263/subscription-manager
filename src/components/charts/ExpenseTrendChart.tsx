...
interface ExpenseTrendChartProps {
  data: MonthlyExpense[]
  categoryData?: CategoryExpenseData[] // New: Data grouped by category
  currency: string
  className?: string
}
...
export function ExpenseTrendChart({ data, categoryData, currency, className }: ExpenseTrendChartProps) {
  ...
  // Get all category names (for grouped histograms)
  ...
  return (
    <Card className={`border-0 shadow-none bg-transparent ${className || ''}`}>
      <CardHeader className="flex flex-row items-center justify-between pb-2 px-6 pt-6">
        <div>
          <CardTitle className="text-lg text-foreground font-bold tracking-wide">{t('chart.expenseTrends')}</CardTitle>
