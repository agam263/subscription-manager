...
export function SubscriptionsPage() {
  ...
  // Apply optimistic updates to filtered subscription list
  ...
  const handleUpdateSubscription = async (id: number, data: Omit<Subscription, "id" | "lastBillingDate">) => {
    // Optimistic update: update UI immediately, then background request and final refresh
    await optimisticUpdateSubscription(
      ...
      async () => {
        ...
        // Background refreshes to ensure consistency with the backend
        fetchSubscriptions()
      },
      (error) => {
        toast({
      ...
      <div className="sticky top-24 z-30 flex flex-col items-center justify-center mb-10 gap-6 w-full max-w-3xl mx-auto md:flex-col mt-4">
        ...
        <div className="w-full flex items-center p-2 rounded-[30px] bg-black/5 dark:bg-black/30 backdrop-blur-2xl border border-border/50 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)]">
          <SearchInput
            ...
            className="w-full border-none bg-transparent shadow-none focus-visible:ring-0 px-6 h-12 text-lg text-foreground placeholder-muted-foreground"
            icon={<Search className="h-5 w-5 text-muted-foreground ml-3" />}
          />
          
          <div className="flex items-center pr-2 gap-2">
            <Popover open={categoryFilterOpen} onOpenChange={setCategoryFilterOpen}>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon" className="relative rounded-full hover:bg-foreground/5 text-foreground/80">
                  <Tags className="h-5 w-5" />
                  {selectedCategories.length > 0 && (
                    <span className="absolute -top-1 -right-1 bg-primary text-primary-foreground rounded-full w-4 h-4 text-[10px] flex items-center justify-center border-2 border-background">
                      {selectedCategories.length}
                    </span>
                  )}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-56 p-0 rounded-[20px] bg-background/90 dark:bg-zinc-900/90 backdrop-blur-xl border-border/50" align="end">
                <div className="p-3">
                ...
                <div className="max-h-72 overflow-y-auto p-1">
                  {usedCategories.map((category) => (
                    <div
                      key={category.value}
                      className={cn(
                        "flex items-center px-3 py-2 rounded-xl transition-all hover:bg-foreground/5 cursor-pointer mb-1",
                        selectedCategories.includes(category.value) && "bg-foreground/5"
                      )}
                    ...
                    >
                      ...
                      <Badge variant="outline" className="ml-auto text-xs bg-transparent border-foreground/20">
              ...
              <PopoverContent className="w-56 p-0 rounded-[20px] bg-background/90 dark:bg-zinc-900/90 backdrop-blur-xl border-border/50" align="end">
                <div className="p-3">
                  <div className="font-medium text-sm flex items-center justify-between">
                    ...
                    {selectedBillingCycles.length > 0 && (
                      <Button variant="ghost" size="sm" className="h-6 text-xs" onClick={() => setSelectedBillingCycles([])}>
                        {t('common:reset')}
                      </Button>
                    )}
                  </div>
                ...
                <div className="max-h-72 overflow-y-auto p-1">
                  {usedBillingCycles.map((cycle) => (
                    <div
                      ...
                      className={cn(
                        "flex items-center px-3 py-2 rounded-xl transition-all hover:bg-foreground/5 cursor-pointer mb-1",
                        selectedBillingCycles.includes(cycle.value as BillingCycle) && "bg-foreground/5"
                      )}
                    ...
                    >
                      ...
                      <Badge variant={getBillingCycleBadgeVariant(cycle.value as BillingCycle)} className="ml-auto text-xs opacity-80">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" className="rounded-full hover:bg-foreground/5 text-foreground/80" onClick={() => setSortOrder(sortOrder === "asc" ? "desc" : "asc")}>
        ...
        {/* Filter Chips with Sliding Highlight */}
        <div className="relative flex items-center p-1.5 rounded-full bg-black/5 dark:bg-black/20 backdrop-blur-2xl border border-border/50 shadow-md overflow-hidden">
          {(['all', 'active', 'trial', 'cancelled'] as const).map((view) => (
            <button
              key={view}
              onClick={() => setCurrentView(view)}
              className={cn(
                "relative px-6 py-2 rounded-full text-sm font-medium transition-colors z-10 w-full md:w-auto",
                currentView === view ? "text-foreground" : "text-muted-foreground hover:text-foreground"
              )}
            >
              {currentView === view && (
                <motion.div
                  layoutId="activeFilter"
                  className="absolute inset-0 rounded-full bg-background dark:bg-white/20 shadow-[0_2px_10px_rgba(0,0,0,0.1)] border border-border/50"
                  transition={{ type: "spring", stiffness: 400, damping: 30 }}
                />
              )}
              <span className="relative z-10 drop-shadow-sm">{t(`subscription:${view}`)}</span>
            </button>
          ))}
        </div>
      </div>
