...
* 
* 1. During the service period (Next payment > current month): As long as the subscription has started, this month should be included in the fee 
* 2. Start and expire in the current month (Next payment = current month and startDate = current month): This month should also be included in the fee 
* 3. Expiration of the current month but not the beginning of the current month (Next payment = current month but startDate < current month): Not included in this month’s fee */
export function isSubscriptionActiveInMonth(
  ...
  // Rule 1: Within the service period (Next payment > current month)
  // As long as the subscription has started, this month should be charged
  if (nextBillingMonthStart > targetMonthStart) {
    return targetMonthStart >= subStartMonthStart;
  }

  // Rule 2: Starts and expires in the current month (Next payment = current month and startDate = current month)
  // This month should also be included in the expenses
  if (nextBillingMonthStart.getTime() === targetMonthStart.getTime()) {
    return subStartMonthStart.getTime() === targetMonthStart.getTime();
  }

  // Rule 3: Due in the current month but not the beginning of the current month (Next payment = current month but startDate < current month)
  // Not included in this month's fee (already processed in the above condition, return false here)
  return false;
}
...
export function getMonthlyExpenses(
  ...
  return Array.from(monthlyMap.entries())
    .map(([monthKey, data]) => {
      ...
      return {
        monthKey, // Keep original monthKey for sorting
...
export function getDateRangePresets(): Array<{ label: string; startDate: Date; endDate: Date }> {
  ...
  return [
    ...
    },
    {
      ...
      endDate: now // Fix: Use current date as end date, make sure current month is included
    },
    {
      label: 'This Year',
