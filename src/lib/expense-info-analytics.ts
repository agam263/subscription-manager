...
function getPeriodDateRange(type: 'monthly' | 'quarterly' | 'yearly', year: number, period: number) {
  ...
  switch (type) {
    case 'monthly': {
      // First day of the month: the 1st day of the current month
      const monthStart = new Date(year, period - 1, 1)
      // End of month: Day 0 of the next month (i.e. the last day of the current month)
...
export function filterRecentExpenses(monthlyExpenses: MonthlyExpense[]) {
  ...
  // Generate all required month keys for quarterly data
